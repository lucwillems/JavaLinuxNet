...
public class IPFactory {
    ...
    public static final Map<Byte,ipv4Factory> ipv4FactoryMap = Collections.unmodifiableMap(new HashMap<Byte, ipv4Factory>() {{
        put((byte)1,   new ipv4Factory() { public IpPacket create(ByteBuffer buffer,int size) { return new ICMPPacket(buffer,size); }});
        put((byte)17,  new ipv4Factory() { public IpPacket create(ByteBuffer buffer,int size) { return new UDPPacket(buffer,size); }});
        put((byte)6,   new ipv4Factory() { public IpPacket create(ByteBuffer buffer,int size) { return new TCPPacket(buffer,size); }});
    }});
